# =============================================================================
# Autor: @opvistar - dez.2021                                                 #
# SETUP LARRY WILLIANS 3 MÉDIAS MÓVEIS                                        #
# DESCRIÇÃO : https://www.youtube.com/watch?v=QXzkrR-dX1E
#==============================================================================
...
###############################     FLUXOGRAMA    #####################################
# ENTRADA - COMPRA NO TOQUE DA MEDIA MOVEL SIMPLES DA MINIMA DOS ULTIMOS 3 CANDLES    #
# FECHAMENTO ABAIXO DA sma21                                                          #
# SAIDA - TOQUE DA MEDIA MÓVEL SIMPLES 3 PERÍODOS DAS MÁXIMAS                         #
######################################################################################
...
# migra arquivos salvos do Tryd (nomenclatura dos arquivos deve ser : <ticker>Tryd.csv)
# exemplo ABEV3 deve ser ABEV3_Tryd.csv

    

#############     PARÂMETROS DE CONFIGURACAO     ################
LISTA_B3 = {"TAEE11","ENBR3", "ITSA4","TRPL4"}  # lista de ativos que vou pegar os dados
PERIODOS_SMA = 21
NUMERO_DE_DIAS = 600     # dias que vai pegar do histórico do Tryd
DESABILITA_VENDA = True  # somente operacões de compra
#############################################vv################  
...
if dict_ativos_precos:
    ...
    for kx_ativo, vx_preco in sorted(dict_ativos_precos.items()):
        ...
        file_relat = open("Resultado_" + datetime.datetime.now().strftime('%Y_%d_%m_%H_%M_%S') + ".txt", "a")  # DEBUG
...
print("Backtest conluido, verifique o arquivo Resultados<data_hora>.txt")    
file_relat.close()    
